      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        {termState && (
          <div className="flex items-center justify-between gap-4 bg-white border border-gray-200 shadow-sm p-4 rounded-xl min-w-[280px]">
            <div><span className="text-[10px] uppercase font-black tracking-wider text-gray-400">Current Session Context</span><h4 className="text-xs font-bold text-gray-900 uppercase mt-0.5">{termState.term} Term · {termState.session}</h4><p className="text-[10px] font-semibold mt-1">Status: <span className={termState.isLocked ? "text-red-600 font-bold" : "text-green-600 font-bold"}>{termState.isLocked ? "🔒 Locked" : "🔓 Open"}</span></p></div>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100 max-h-64 overflow-y-auto shadow-md bg-white mt-2 relative z-10">
            {searchResults.map((student) => (
              <div key={student._id} onClick={() => openHistoryModal(student._id)} className="flex items-center justify-between p-4 hover:bg-gray-50 cursor-pointer transition-all group">
                <div className="flex items-center gap-3"><div className={`p-2.5 rounded-xl ${student.isActive ? 'bg-indigo-50 text-indigo-600' : 'bg-amber-50 text-amber-600'}`}><GraduationCap size={20} /></div><div><p className="text-sm font-black text-slate-800">{student.firstName} {student.lastName}</p><p className="text-[11px] text-slate-500 font-bold mt-0.5">Admission Number: <span className="font-mono text-slate-700 font-black">{student.admissionNumber}</span> · Class: <span className="text-slate-700 font-black">{student.class}</span></p></div></div>
      ...
      {isModalOpen && selectedStudentId && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-gray-50 rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative border border-gray-100 pt-6">
            <button onClick={closeHistoryModal} className="absolute top-6 right-6 bg-white hover:bg-gray-100 border border-gray-200 text-gray-700 px-3.5 py-1.5 rounded-xl font-bold text-xs shadow-sm z-50">✕ Close Archive</button>
